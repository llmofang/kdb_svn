/Danny Greening
/danny.greening@aquaq.co.uk
/09.01.2014

/Aquaq Analytics
/www.aquaq.co.uk
/email: info@aquaq.co.uk
/The latest version of this and other useful scripts can be found at www.aquaq.co.uk/q_code_contributions


/This script set up the framework to keep the results of recently run functions in a table in memory.
/This is useful as it allows for function calls that are executed repeatedly to be calculated once and the result to be referenced for every call after the first time.
/Both the function called and the results of the function are recorded separately and are given a unique id number.
/The maximum size of the table of results and of any individual result are both limited.
/If an individual result is too big a message is added that it failed to be added to the results table.
/If a result is to be added that would cause the table to exceed the maximum table size then the results with the greatest time since they have been accessed are removed until there is enough space for the new result.
/The desired age of a result can also be specified so if a result in the table is too old it will be regenerated with the same id number.
/A running log is also kept of every function call to the results table and whether a result is added, removed or referenced.
