...
using System.Diagnostics.CodeAnalysis; //csc c.cs  given >q trade.q -p 5001
namespace kx
{
 ...
 new Month(ni),new Date(ni),new DateTime(0),new KTimespan(nj),new Minute(ni),new Second(ni),new TimeSpan(ni)};
        ...
        {
            public TimeSpan t; private KTimespan() { }public KTimespan(long x) { t = new TimeSpan(x == nj ? nj : x / 100); }
